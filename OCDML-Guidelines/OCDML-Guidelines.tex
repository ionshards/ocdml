...
% @Cory - We would want every orientation signal to trigger a config link, no?

Examples of the former type are enumerated in \cref{ex:non_trigger_examples}.

\eenumsentence{
    \item On the large table \emph{stands} a lighted lamp.
    \label{ex:lamp_stands}
    \item \ldots \emph{lies} down on sofa.
    \label{ex:lies_sofa}
...
\paragraph{\texttt{trigger}} % fo;d
\label{par:trigger}
This attribute takes the ID value of the \texttt{orientation\_signal} that triggered the \texttt{configuration\_link}.  Since creating an \texttt{orientation\_signal} automatically commits you to creating a \texttt{configuration\_link}, each link is required to have a value for the \texttt{trigger} attribute.  Take \cref{ex:annot_stands}, the \textsc{orientation\_signal} \signal{stands}{1} is the \texttt{trigger} for the \textsc{configuration\_link} between \emph{table} and \emph{lamp}.

% paragraph trigger (eng)

\paragraph{\texttt{direction}}
\label{par:direction}
This attribute is used to indicate the asymmetrical directionality of the relation in terms of figure-to-ground or ground-to-figure. 
...
Provided below, in \cref{ex:config_links}, are examples of \textsc{configuration\_links}.

\enumsentence{
}\label{ex:config_links}
